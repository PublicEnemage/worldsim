...
**Last updated:** 2026-07-03 (G3 confirmed — Bayesian posterior, meaninglessness threshold, BandResult fields; integration PR #1617 → release/m19; G4 #1529 coordination gate unblocked)**Current milestone:** M19 — Constraint Search and Empirical Calibration
...
| Active wave | Wave 2 — G3 confirmed (integration PR #1617 auto-merging); G2D pending ADR-020; G4 coordination gate cleared |
| Active sprint groups | G2D — pre-gate (ADR-020 required); G4 — coordination gate cleared, entry next |
| Active sprint journal issues | #1587 (G3 — closed at exit) |
---
...
| #1553 | Iceland 2008–11 Type A+B | G2D Wave 2 | Medium — blocked: ADR-020 acceptance || #1528 | PSP driver arc + auditability panel (DEMO-165) | G4 Wave 2–3 | High |
...
| G4 #1529 CI label text for `PRE_CALIBRATION_PROVISIONAL_DIRECTIONAL` state must land | G3 north star test / PI exit §6 | Demo 8 Act 2 |
...
- **ADR-020 (ARCH-014):** Emergency instrument economic transmission pattern — capital controls channels. EL confirmed M19 scope 2026-07-03. Architect Agent activates to author ADR-020; G2D sprint entry files after acceptance. G2C integration PR defers to G2D exit per sprint entry §1.5.- **NM-086:** E2E mock routes must be verified against `api_contracts.yml` before the implementation PR opens (G1, filed 2026-07-03). Process: QA Lead mock-helper verification is a blocking checklist item on intent authorship.
