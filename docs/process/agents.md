...
- **SPRINT:** Sprint planning for a named milestone. PM consults BPO (value prioritization), FA (file area grouping), CE (dependency sequencing), and Ar (ADR prerequisites) before producing a draft. Output: a sprint plan document at `docs/process/sprint-plans/{milestone-slug}-sprint-plan.md` plus a `release/m{N}` branch created from `main`. EL approves the plan before implementation begins. Full SOP: `docs/process/sprint-planning-sop.md`. **Status update obligation:** after each sprint group's PR merges to the release branch, PM updates the sprint plan document to mark that group complete (status `DONE`, merge date noted). The sprint plan is a living artifact — not a one-time document.
...
- **HORIZON skip audit scope — conditional guards included (QA-NM027-F1).** The HORIZON skip audit covers `test.skip`, `test.fixme`, AND any E2E test containing a conditional assertion guard whose falsehood produces a no-op: `if (value !== null)`, `if (hasElement)`, `if (count > 0)`, or structurally equivalent. Each such guard is enumerated, assigned a documented unblock condition naming the component and implementation detail required, and given a milestone expiry. A guard without a documented unblock condition is an orphaned obligation — same severity as an undocumented `test.skip`.
- **Guarded test implementation dependency record (QA-NM027-F2).** When I author a test whose guard depends on a specific implementation detail in another agent's component — a `performance.measure` call, a `data-testid`, a specific event or state shape — I record the dependency explicitly before the session ends. Either an inline comment in the test naming the component and required detail, or a GitHub issue assigned to the implementing agent. The guard is not complete until the dependency is recorded and the implementing agent is aware. An unrecorded dependency is an orphaned assumption.
- **No-op guard locator audit (NM-058 addition).** At every sprint entry, run `grep -rn "isVisible().catch" frontend/tests/e2e/` to identify all no-op guard patterns. For each match, verify the locator testid against `grep -rn "data-testid" frontend/src/`. Any guard whose testid does not appear in the current source is a filing-blocking finding — raise with the Frontend Architect before the sprint entry is approved. This audit takes under two minutes. It would have caught the AC-009 testid mismatch (`mode-3-activate` vs `mode3-toggle`) at every sprint entry since M12 (NM-058, fourth NM-027 pattern recurrence).
- **Performance measurement methodology audit (NM-059 addition).** At every sprint entry, run `grep -rn "performance.mark" frontend/tests/e2e/` to identify all performance-gated tests. For each match, verify that both the start and end marks — and the action being measured — are contained within a single `page.evaluate` call. Any test that places a Playwright `waitForTimeout`, `click()`, or separate `page.evaluate` between its start and end marks is a filing-blocking methodology finding. Multi-CDP methodology produces CI-runner-load-dependent results, not render measurements (179ms → 802ms variance on consecutive runs — NM-059). Valid pattern: single `page.evaluate` with both marks + RAF wait for React commit cycle. No CDP round-trips inside the measurement window.
- **Setup completeness audit for conditional-render tests (NM-061 addition).** At every sprint entry, identify E2E tests that: (1) create scenario or entity data via direct `fetch` API calls, AND (2) then check for conditional UI component visibility via `isVisible()`. For each such test, read the render condition in the source component (e.g. `activeScenarioDetail?.configuration?.projection_steps > 8`) and verify the test's setup flow satisfies that condition through the UI — not just through API state. API-created data that is never surfaced through a UI selection action (Scenarios panel → "Select as primary scenario" click, or equivalent) will not satisfy conditional renders that depend on `activeScenarioDetail`, `selectedScenarioId`, or equivalent UI-driven state. A test whose guard fires because the setup never triggered the render condition is a filing-blocking finding, identical in effect to an NM-058 testid mismatch. (NM-061: AC-F8 `human-capital-trajectory-panel` guard — panel condition `projection_steps > 8` never true because scenario created via API but not selected in UI.)
