...
## NM-087 — Shared-State File Changes Lost on Branch Switch; Session Summary Permanent Artifact Unwritten for 24 Hours; Caught by Human Recall (Reactive)
...
NM-075 addresses the inverse problem (implementation file changes lost on branch switch via worktree isolation). NM-087 is the complementary gap: shared-state file changes written to session memory but not yet committed.
