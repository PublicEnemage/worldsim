...
| ARCH-010 | TBD | Scenario Grounding Architecture — pre-creation data quality preview, scenario grounding strip (initial state + source + vintage), Fidelity panel contextualisation, parameter persistence | 2026-06-15 | ASSIGNED — ADR-016 | 16 | M14 | Evidence: live UI input confidence audit 2026-06-15 (`docs/demo/m14/reviews/2026-06-15-ux-input-confidence-audit-minister-exercise.md`). Gap taxonomy IC-1 through IC-7. M14 Wave 1 — ADR-016 must be authored and EL-accepted before ADR-015 (Wave 2) implementation begins. Panel: Architect Agent (author), UX Designer Agent (sign-off required), Frontend Architect Agent (C), Chief Methodologist (C — data provenance API design), Business PO (C), Engineering Lead (A). |
