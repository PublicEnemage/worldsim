...
| ARCH-011 | #845 | Zone 1A information architecture — multi-dimensional encoding across framework × entity × branch × mode; primary cognitive task per mode (Personas 1/2/5); cohort disaggregation path | 2026-06-20 | ASSIGNED — ADR-017 | 17 | M15 | Evidence: Phase 1 design thinking doc `docs/ux/design-thinking/zone-1a-information-architecture.md` (M14 G6c, PR #1033). Tier 1 ADR — UX Designer independent sign-off required. Panel: Architect Agent (author), UX Designer (independent sign-off, NM-042), Frontend Architect (C), Business PO (C), Customer Agent (Layer 3), Chief Methodologist (C — composite aggregation), EL (A). Architecture Review: ARCH-REVIEW-007-milestone15.md (Phase 2, 2026-06-22). ADR-017 authorship complete 2026-06-22. |
| ARCH-012 | #1252 | Zone 1B proportional allocation — explicit sub-zone A/B split replacing flex negotiation; MDA alert panel guaranteed minimum; CohortImpactSection internal scroll | 2026-06-25 | ASSIGNED — ADR-018 | 18 | M17 | G3 Phase 2. Path B determination: overflow contract + multi-occupant allocation are new decisions not derivable from ADR-017 (Zone 1A) or ADR-014 (predates CohortImpactSection; "Valid Until" clause fires). Panel: Architect Agent (author), Frontend Architect Agent (C), UX Designer Agent (sign-off, Tier 2), EL (A). Supersedes PR #1235 temporary minHeight:80px guarantee. ADR-018 authorship 2026-06-25. |
| ARCH-013 | #1360 | Control Plane Column — Mode 2 and Mode 3 implementation: column content by mode, blue/orange cross-layer visual system, shock type taxonomy, history list contracts, sizing constraints, mode transition behavior | 2026-06-26 | ASSIGNED — ADR-019 | 19 | M18 | Evidence: control plane design package (#1354); current state audit (Artifact 1, #1355); target state specification (Artifact 2, #1356); delta analysis (Artifact 4, #1358); scope decision (Artifact 5, #1359). Tier 1 — UX Designer independent sign-off required (NM-042). Panel: Architect Agent (author), UX Designer Agent (independent sign-off, separate session), Frontend Architect Agent (C), Business PO (C), Customer Agent (C — Layer 3 on record from Artifact 3 #1357), EL (A). ADR may not be initiated until Artifacts 2, 4, and 5 are on record. |
