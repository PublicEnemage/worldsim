...
| ARCH-014 | #1532 | Emergency instrument economic transmission pattern — capital controls ExternalSectorModule (reserve protection) + MacroeconomicModule (credit contraction) channels; DemographicModule dead subscription bug | 2026-07-02 | ACCEPTED — ADR-020 (2026-07-03) | 20 | M19 | Accepted 2026-07-03. 5 INCORPORATE items applied (CE: γ declaration, log-before-raise; CM: Iceland ε SBA confound, β default 0.020; DE: Q1+Q2 scope note). CM calibration deliverable is pre-G2D implementation PR gate. CE audit (Decision 3) required before G2D implementation PR. G2D sprint entry (#1553) now unblocked. Panel: Architect (author), CE ✓, CM ✓, Development Economist ✓, Geopolitical Analyst ✓ (unconditional), UX Designer ✓ (Tier 2 trace), EL ✓. Panel review: `docs/adr/reviews/ADR-020-panel-review.md`. |
