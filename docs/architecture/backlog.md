...
| ARCH-011 | #845 | Zone 1A information architecture — multi-dimensional encoding across framework × entity × branch × mode; primary cognitive task per mode (Personas 1/2/5); cohort disaggregation path | 2026-06-20 | PENDING_NUMBER | — | M15 | Evidence: Phase 1 design thinking doc `docs/ux/design-thinking/zone-1a-information-architecture.md` (M14 G6c, PR #1033). Tier 1 ADR — UX Designer independent sign-off required. Panel minimum: Architect Agent (author), UX Designer (independent sign-off), Frontend Architect (C), Business PO (C), Customer Agent (Layer 3 assessment), EL (A). Architecture Review (ARCH-REVIEW) required before authorship. Assign ADR-017 when Architect Agent claims the number at authorship time. |
