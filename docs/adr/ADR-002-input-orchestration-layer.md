...
**Last Reviewed:** 2026-06-10 — M12 exit review (SCAN-026). Renewal trigger fired: `BilateralTradeShock` added in M12 G5 (Issue #751, ADR-012) is a new `ControlInput` subclass — a ControlInput type taxonomy addition. Assessment: additive extension consistent with the existing extensibility contract. `BilateralTradeShock` follows the established pattern: inherits from `ControlInput`, implements `to_events()` producing events on the target entity using existing `MeasurementFramework` values (`FINANCIAL`, `HUMAN_DEVELOPMENT`), deserialized via `_deserialize_control_input("BilateralTradeShock")`. The audit trail contract is unchanged — `ControlInputAuditRecord.input_type` records `"BilateralTradeShock"` alongside existing subclass names. No existing subclasses modified or removed. No audit trail schema changes. No uncertainty quantification standard changes. No amendment to decision body required — the existing extensibility contract accommodates this naturally. License renewed to Milestone 13.
...
The Argentina 2001-2002 fixture (Issue #192) introduces `imf_program_acceptance`
...
are unaffected. License renewed for Milestone 5.
